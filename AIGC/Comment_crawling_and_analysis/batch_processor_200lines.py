...
def analyze_reviews(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        reviews = [line.strip() for i, line in enumerate(f) if line.strip() and i < 200]
    ...
    total = len(results)
    print(f"\n处理完成，共分析{total}条有效评论，结果已保存到{output_file}")
    print("\n=== 情感分布统计 ===")
...
if __name__ == '__main__':
    ...
    output_csv = r'AIGC\Comparison_of_similar_products_and_external_link_information\analysis_result_200lines.csv'  # 输出文件路径
